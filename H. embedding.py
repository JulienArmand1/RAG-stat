...
if __name__ == "__main__":
    ...
    # Merge des datasets
    imput_dir = r"C:\Users\14388\Desktop\Projet de session"
    path_data = os.path.join(imput_dir, "table_chunks.xlsx")
    chunks = pd.read_excel(path_data)
    path_equiv = os.path.join(imput_dir, "Equivalence_complete.xlsx")
    ...
    chemin_df = os.path.join(imput_dir, "Chunk_mod.xlsx")
    df_left.to_excel(chemin_df)

    output_dir = r"C:\Users\14388\Desktop\Projet de session\embeddings v6"
    for annee in range(1975,2025):
        path = os.path.join(imput_dir, "pdf v6", str(annee))
        if os.path.isdir(path):
            vector = chunks_to_embedding(list(df_left[df_left['fichier'].str.contains(str(annee))]['chunks']), 1500)
