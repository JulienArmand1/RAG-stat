...
if __name__ == "__main__":
    ...
    for index, row in question.iterrows():
        ...
        if index>=69:
            ...
            list_nbs_chunks =(
                [[i,texte,fichier,"court","llama"] for i in [2,8]]+
                [[i,texte,fichier,"long","llama"] for i in [2,8]]+
    ...
    columns = (
        ["court_"+"llama_"+str(i) for i in [2,8]]+
        ["long_"+"llama_"+str(i) for i in [2,8]]+
    ...
    df1.to_excel(os.path.join(analyse_dir, 'analyse500_score_test_2.xlsx'), index=False)

    df2 = pd.DataFrame(array2, columns=columns)
    df2.to_excel(os.path.join(analyse_dir, 'analyse500_indicatrice_test_2.xlsx'), index=False)

    df3 = pd.DataFrame(array3, columns=columns)
    df3.to_excel(os.path.join(analyse_dir, 'analyse500_longueur_test_2.xlsx'), index=False)