...
if __name__ == "__main__":
    ...
    df1.to_excel(os.path.join(analyse_dir, 'analyse500_score_test.xlsx'), index=False)

    df2 = pd.DataFrame(array2, columns=columns)
    df2.to_excel(os.path.join(analyse_dir, 'analyse500_indicatrice_test.xlsx'), index=False)

    df3 = pd.DataFrame(array3, columns=columns)
    df3.to_excel(os.path.join(analyse_dir, 'analyse500_longueur_test.xlsx'), index=False)